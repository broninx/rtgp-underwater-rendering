...
public:
    ...
    GLfloat MovementCompensation = 0.1f;
    ...
    {        
        ...
        if (this->Position.x >= worldSize - limit )
            this->Position.x = worldSize - limit;

        if (this->Position.z >= worldSize - limit)
            this->Position.z = worldSize - limit;

        if (this->Position.y < worldHeight + Up.y + 2.0f)
            this->Position.y = worldHeight + Up.y + 2.0f;

        if (this->Position.y >= worldHeight + worldSize /2)
            this->Position.y = worldHeight + worldSize / 2;
    }
